...
export const obtenerResultadosBusquedaEnElRegistroDelElemento = async (data) => {
    try {
       ...
       ${sqlDinamicoConstructor(nombreColumna, sentidoColumna)}
        ...
        const parametros = [
            terminosFormateados,
            numeroPorPagina,
            numeroPagina(numeroPagina_humano),
            elementoUID

        ]
        const resuelve = await conexion.query(consultaConstructor, parametros);
