const casaVitini = {
        ...
        navegacion: {
            ...
            controladorVista: async (entrada) => {
                ...
                if (!pantallaDeTransicionRenderizada) {
                    ...
                    casaVitini.ui.componentes.pantallaDeCargaSuperPuesta(configuracionPantallaCarga)
                }
    ...
    ui: {
            ...
            alojamiento: {
                ...
                resumen: {
                    arranque: async () => {
                        ...
                        botonPreConfirmar.addEventListener("click", casaVitini.ui.vistas.alojamiento.resumen.preConfirmar)
                        ...
                        */
                        ...
                        // const totalesPorApartamento = desgloseFinanciero.entidades.reserva.desglosePorApartamento
                    ...
                    obtenerPrecioReserva: async () => {
                        ...
                        if (reservaNoConfirmada) {
                            ...
                            if (respuestaServidor?.ok) {
                                return respuestaServidor?.ok.desgloseFinanciero
                            }
                        }
                    ...
                    reservas: {
                        ...
                        UIComponentes: {
                            ...
                            "propuestaEliminarPernoctanteUI": (propuesta) => {
                                ...
                                botonOpcionesCliente.setAttribute("habitacionUID_", habitacionUID)
                                botonOpcionesCliente.innerText = "Cancelar"
                                //botonOpcionesCliente.addEventListener("click", casaVitini.shell.controladoresUI.limpiarAdvertenciasInmersivas)
                                bloquePropuestaCambio.appendChild(botonOpcionesCliente)
                                //document.querySelector("[componente=buscadorRapido]").style.display = "none"
        ...
        componentes: {
            ...
            errorConexion_noIvasivo: (advertencia) => {
                ...
                const info = document.createElement("div")
                info.setAttribute("class", "errorInfo")
                info.innerText = "No se puede establecer conexion con el servidor. Por favor verifica que estas conectado y reintentalo. Se ha producido un error en la red y no se ha podido comunicar con el servidor, si es por una causa circunstancial de la red, reintentalo y deberia funcionar. Comprueba que tienes acceso a la red. Por ejemplo, comprueba si puedes acceder a google.com o hacer un ping a google.com o a otros sitios conocidos. Si tienes acceso a sitios conocidos es probable que el problema este en el servidor de Casa Vitini."
                marcoElastico.appendChild(info)
                const boton = document.createElement("div")
                boton.setAttribute("class", "errorBoton")
                boton.innerText = "Volver"
            ...
            calendario: {
                constructorCalendarioNuevo: (metadatos) => {
                    try {
                        ...
                        if (!instanciaUID_contenedorFechas) {
                            const error = "El contructor del calendario necesita una instanciaUID_contenedorFechas para calendario"
                            throw new Error(error)
                        }
                ...
                configurarCalendario: async (data) => {
                    ...
                    const fechaEntradaVolatil_Humana = areaContenedorFechas.querySelector("[calendario=entrada]").
                        getAttribute("memoriaVolatil")
                    const fechaEntradaAarray = fechaEntradaVolatil_Humana?.split("/") ? fechaEntradaVolatil_Humana?.split("/") : []
                    const diaSeleccionadoEntrada = Number(fechaEntradaAarray[0])
                    const mesSeleccionadoEntrada = Number(fechaEntradaAarray[1])
                    const anoSeleccionadoEntrada = Number(fechaEntradaAarray[2])

                    const fechaSalidaVolatil_Humana = areaContenedorFechas.querySelector("[calendario=salida]").getAttribute("memoriaVolatil")

                    const fechaSaliraArray = fechaSalidaVolatil_Humana?.split("/") ? fechaSalidaVolatil_Humana?.split("/") : []
                    const diaSeleccionadoSalida = Number(fechaSaliraArray[0])
                    const mesSeleccionadoSalida = Number(fechaSaliraArray[1])
                    const anoSeleccionadoSalida = Number(fechaSaliraArray[2])
                    ...
                    if (rangoIDV === "inicioRango") {
                        ...
                        if (fechaEntradaVolatil_Humana) {
                            ...
                            resolverCalendario.diaSeleccionado = diaSeleccionadoEntrada
                            resolverCalendario.mes = Number(mesSeleccionadoEntrada)
                            resolverCalendario.ano = Number(anoSeleccionadoEntrada)
                ...
                navegacionCalendarioNuevo: async (calendario) => {
                    ...
                    const calendarioResuelto = await casaVitini.ui.componentes.calendario.resolverCalendarioNuevo(calendarioContruir)
                ...
                calendarioCompartido: {
                    seleccionarDia: (dia) => {
                        ...
                        if (calendarioIO === "entrada") {
                            ...
                            if (fechaSalidaVolatil_Humana) {
                                if (fechaSalidaSeleccionada.mes === Number(mesSeleccionado) && Number(anoSeleccionado) === fechaSalidaSeleccionada.ano) {
                                    selectorDias.forEach((dia) => {
                                        if ((Number(dia.getAttribute("dia")) > diaSeleccionado)
                                            &&
                                            (Number(dia.getAttribute("dia")) < fechaSalidaSeleccionada.dia)) {
                        ...
                        if (calendarioIO === "salida") {
                            ...
                            if (fechaEntradaVolatil_Humana) {
                                if (fechaEntradaSeleccionada.mes === Number(mesSeleccionado) && Number(anoSeleccionado) === fechaEntradaSeleccionada.ano) {
                                    selectorDias.forEach((dia) => {
            ...
            contenedorFinanciero: {
                ...
                control: (data) => {
                    try {
                        ...
                        if (modoUI !== "plaza" && modoUI !== "administracion") {
                            throw new Error("El constructor del contenedor financiero necesita modoUI en plaza o administracion")
                        }
                ...
                componentesUI: {
                    ...
                    ofertas: {
                        ...
                        ofertaUI: function (data) {
                            ...
                            const ofertasPorAdministradorArray = Object.keys(ofertasPorCondicion)
                            const contenedorFinanciero = document.querySelector(destino).querySelector("[contenedor=financiero]")
                            const modoUI = contenedorFinanciero.getAttribute("modoUI")
                            const instanciaUID = data.instanciaUID

                            const contenedorOfertas_selector = contenedorFinanciero.querySelector("[contenedor=ofertas]")
                            const ofertasEnTotal = ofertasPorCondicionArray?.length > 0 || ofertasPorAdministradorArray?.length > 0

                            if (!contenedorOfertas_selector && ofertasEnTotal) {
                                const contenedor = document.createElement("div")
                            ...
                            const selectorContenedorOferta = contenedorOfertas_renderizado.querySelectorAll("ofertaUID")
                            selectorContenedorOferta.forEach((ofertaObsoleta) => {
                            ...
                            if (ofertasPorCondicionArray.length === 0 && ofertasPorAdministradorArray.length === 0) {
                                contenedorOfertas_renderizado?.remove()
                            }
                        ...
                        componentesUI: {
                            globalUI: function (data) {
                                ...
                                if (!contenedorOfertaUI_selector) {
                                    ...
                                    document.querySelector(destino).querySelector("[contenedor=financiero]").querySelector("[contenedor=ofertas]").querySelector(`[contenedor=${destinoOrigenOferta}]`).appendChild(contenedorOfertaUI)
                            ...
                            condicionesUI: (data) => {
                                ...
                                condicionesArray.forEach((condicion) => {
                                    ...
                                    // const contendorCondicion_selector = document.querySelector(destino)
                                    //     .querySelector("[contenedor=ofertas]")
                                    //     .querySelector(`[contenedor=${destinoOrigenOferta}]`)
                                    //     .querySelector("[contenedor=condiciones]")
                                    //     .querySelector(`[tipoCondicion=${tipoCondicion}]`)

                                    // if (!contendorCondicion_selector) {
                                    //     const contendorCondicion = document.createElement("div")
                                    //     contendorCondicion.classList.add("contenedorCondicion")
                                    //     contendorCondicion.setAttribute("tipoCondicion", tipoCondicion)
                                    ...
                                    //         .querySelector(`[contenedor=${destinoOrigenOferta}]`)
                                    //         .querySelector("[contenedor=condiciones]")
                                    //         .appendChild(contendorCondicion)
                                    // }
                                    const contendorCondicion = document.createElement("div")
                                    contendorCondicion.classList.add("contenedorCondicion")
                                    contendorCondicion.setAttribute("tipoCondicion", tipoCondicion)
                                    ...
                                    document.querySelector(destino)
                                        ...
                                        .querySelector("[contenedor=condiciones]")
                                        .appendChild(contendorCondicion)
                                    // const contendorCondicion_renderizado = document.querySelector(destino)
                                    //     .querySelector("[contenedor=ofertas]")
                                    //     .querySelector(`[contenedor=${destinoOrigenOferta}]`)
                                    //     .querySelector("[contenedor=condiciones]")
                                    //     .querySelector(`[tipoCondicion=${tipoCondicion}]`)
                                    ...
                                    contenedorCondiciones_renderizado.appendChild(contendorCondicion)
                            ...
                            descuentosUI: (data) => {
                                ...
                                if (tipoDescuento === "porRango" && subTipoDescuento === "porDiasDelRango") {
                                    ...
                                    descripcionDescuento.innerText = `Esta oferta aplica un descuento a los dias de la reserva. A continuación se detallan los dias donde la oferta aplica descuentos y que tipo de desceuntos aplica en cada dia.`
                                    ...
                                    for (const descuentoPorDia of descuentoPorDias) {
                                        ...
                                        if (tipoDescuentoPorDia === "netoPorDia") {
                                            ...
                                            if (tipoAplicacion === "porcentaje") {
                                                descripcionDescuentoTotalNeto.innerText = `Esta oferta aplica un descuento del ${descuentoTotal}% a los netos de los dias de la reserva que estan dentro del rango establecido entre en el ${fechaInicioRango_ISO} y el ${fechaFinalRango_ISO}.`
                                        ...
                                        contenedorDescuentos_renderizado.appendChild(contenedorDia)
                        ...
                        porTotal: (data) => {
                            ...
                            contenedorPorTotal_renderizado.innerHTML = null
                            ...
                            contenedorPorTotal_renderizado.appendChild(tituloContendor)
                            ...
                            porTotal.forEach((detallesDelTotal) => {
                                ...
                                contenedorPorTotal_renderizado.appendChild(contenedorDescuentoTotal)
                        ...
                        entidades: {
                            ...
                            reserva: (data) => {
                                ...
                                Object.entries(porApartamento).forEach(([apartamentoIDV, descuentosDelApartamento]) => {
                                    const selector = document.querySelector(destino).querySelector("[contenedor=financiero]").querySelector("[contenedor=porApartamento]").querySelector(`[apartamentoIDV="${apartamentoIDV}"]`)
                                    ...
                                    totalDescuentosAplicadosUI.innerText = `${totalDescuentosAplicados}$ Suma total de descuentos aplicados al apartamento`
                                    ...
                                    selector.appendChild(contenedorDescuentosPorApartamento)
                                ...
                                Object.entries(porDia).forEach(([fechaNoche, detallesDelDia]) => {
                                    ...
                                    Object.entries(porApartamento).forEach(([apartamentoIDV, desgloseDelApartamento]) => {
                                        ...
                                        const contenedorDescuentosGlobal_selector = selectorApartamentoDelaNoche.querySelector("[contenedor=descuentosGlobal]")
                                        if (!contenedorDescuentosGlobal_selector) {
                                            ...
                                            const titulo = document.createElement("div")
                                            titulo.classList.add("negrita")
                                            titulo.innerText = "Descuentos aplicados al total neto del apartamento"
                    ...
                    impuestos: (data) => {
                        ...
                        impuestos.forEach((impuesto) => {
                            ...
                            if (modoUI === "administracion") {
                                ...
                                botonEliminarImpuesto.innerText = "Eliminar impuesto"
                    ...
                    totales: (data) => {
                        ...
                        if (!contenedorTotales_selector) {
                            ...
                            if (modoUI === "administracion") {
                                ...
                                botonInsertarDescuento.innerText = "Reconstruir instantaneas del contenedor financiero"
                                botonInsertarDescuento.addEventListener("click", () => {
    ...
    utilidades: {
        ...
        fechas: {
            constructorObjetoEstructuraPrecioDia: (fechaEntrada_ISO, fechaSalida_ISO) => {
                const arregloFechas = []
                let fechaEntrada_Objeto = new Date(fechaEntrada_ISO)
                const fechaSalida_Objeto = new Date(fechaSalida_ISO)
